...
sam package --region us-east-1 --s3-bucket buildkite-serverless-apps-us-east-1 --s3-prefix elastic-ci/agent-scaler-arm64 --output-template-file packaged-arm64.yml
# Restore original filenames
mv handler.zip handler-arm64.zip
mv handler-x86_64-backup.zip handler.zip
# Update the architecture in the arm64 template
sed -i.bak 's/- x86_64/- arm64/g' packaged-arm64.yml && rm packaged-arm64.yml.bak

...
func main() {
	...
	count, err := getScheduledJobCount(*buildkiteApiToken, *buildkiteOrg, *queue)
	if err != nil {
		log.Fatal(err)
	}
	...
	currentCount, err := getAgentCount(sess, *agentCluster, *agentService)
	if err != nil {
		log.Fatal(err)
	}
	...
	if err = updateAgentCount(sess, *agentCluster, *agentService, count); err != nil {
		log.Fatal(err)
	}

	resources, err := getContainerInstanceResources(sess, *agentCluster)
	if err != nil {
		log.Fatal(err)
	}
